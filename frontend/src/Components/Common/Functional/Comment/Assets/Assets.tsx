...
import { assetType } from '../../Posts/PostForm/PostForm'
import { VideoPlayer } from '../../Video/VideoPlayer/VideoPlayer'

import classes from './Assets.module.scss'

export const CommentAssets = ({audio,video,image}:assetType) => {
  ...
  return (
    ...
    <>
     {(audio?.entity?.url || video?.entity?.url || image?.entity?.url) && <div className={classes.assets}>
      {(video?.entity?.url || image?.entity?.url) && <div className={classes.top}>
        <div
        ...
        >
          {showVideoPad && (
            <CommonPad>
              ...
              <DeleteOutlined
                onClick={() => {
                  video.operation_type ='delete/video_f_post_u';force((prev)=>!prev)
                }}
                className={classes.removeInModal}
              ></DeleteOutlined>
            </CommonPad>
        ...
        <CommonModal setActive={setShowVideoPlayer} active={showVideoPlayer}>
          <VideoPlayer src={video?.entity?.url}></VideoPlayer>
        </CommonModal>
      </div>}
      ...
      {audio?.entity?.url && 
      <>
       <DeleteOutlined className={classes.deleteAudioIcon}  onClick={()=>{audio.operation_type='delete/audio_f_post_u' ;force((prev)=>!prev)}}></DeleteOutlined>
      
      <AudioComponent src={audio?.entity?.url}></AudioComponent>
      </>
