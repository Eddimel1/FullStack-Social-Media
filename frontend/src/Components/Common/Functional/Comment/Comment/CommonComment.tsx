import { LikeOutlined, FrownOutlined, EyeOutlined, RestOutlined } from '@ant-design/icons'
...
export const CommonComment = React.memo(({ comment }: { comment: commentT_U }) => {
  ...
  const [removeComment,removed_comment] = useRemoveCommentForPost_U_Mutation({update:(cache,data)=>{
  ...
  return (
    <>
      ...
      <div className={classes.wrapper}>
          ...
          <div className={classes.vertical_section2}>
            ...
            <div className={classes.center}>
              ...
              >
                ...
                <div className={classes.metaItem} onClick={()=>setShowReplyForm((prev)=>!prev)}>reply</div>
        ...
        <div
          style={{ display: 'flex', justifyContent: 'center', width: '100%',cursor:'pointer' , margin:'2rem 0'}}
          onClick={() => setShowReplies((prev) => !prev)}
        >
          Show Replies...
        </div>
        { confirmModal && <>
        <CommonConfirm showModal={showModal} title='Are you sure you want to remove your comment?' >
     <Button onClick={() => {
        removeComment({variables:{input:{commentId:Number(comment.id),ownerId:comment.ownerId}}}).catch((e)=>console.log(e))
       ...
       {isShowVideoPlayer && <CommonModal setActive={ setShowVideoPlayer} active={isShowVideoPlayer}>
        ...
        {showReplyForm &&  <div style={{margin:'2rem'}}><ReplyForm isRootReply={true} _openReplies={setShowReplies} receiverId={comment.user.id}  commentId={Number(comment.id)}></ReplyForm></div>}
         {showReplies &&  <ReplySection  commentId={Number(comment.id)}></ReplySection>}
