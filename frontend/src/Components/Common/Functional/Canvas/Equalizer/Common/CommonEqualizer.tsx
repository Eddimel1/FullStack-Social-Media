...
export const CommonEqualizer = ({audioRef,css}:_props) => {
          ...
          if (
            ...
            for (let i = 0; i < dataArray.current.length; i++) {
              ...
              if(dataArray.current[i] % 2 === 0){
                ...
                ctx.strokeStyle = `rgb(${Math.random()*255},${bit},${Math.random()*255})`
                ctx.fillStyle = `rgb(${Math.random()*255},${Math.random()*255},${bit}})`
                canvasRef.current.style.backgroundColor = `rgba(${bit}},${Math.random()*255},${Math.random()*255},0.1)`
  ...
  return (
    <canvas style={{...css}} ref={canvasRef}></canvas>
  )
}
