...
@Injectable()
export class ImageCommentService_F_G {
  constructor(
    @InjectRepository(Image_F_Comment_F_Photo)
    private readonly imageCommentRepository: Repository<Image_F_Comment_F_Photo>,
  ) {}

  async insertImage(comment_id: number, url: string, filename: string) {
    const image = new Image_F_Comment_F_Photo()
  ...
  async deleteImageByOwnerId(comment_id: number, file_name: string) {
    const isSuccess = await this.imageCommentRepository
      .createQueryBuilder(COMMENT_RELATIONS.IMAGES)
      .delete()
