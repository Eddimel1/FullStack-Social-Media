...
import { BaseCRUDService_WO_Owner } from 'src/generics/generic-services/base-crud.service'
...
export class GroupInfoService extends BaseCRUDService_WO_Owner<GroupInfo> {
  ...
  async create(createGroupInfoInput: CreateGroupInfo_I) {
    ...
    const id = (await this.repository.save(new_group_info)).id
    const updated_info = await this.repository.findOneBy({ id })
