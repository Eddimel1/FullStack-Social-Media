import { ObjectType, Field } from '@nestjs/graphql'
import { BaseCommentEntity } from 'src/typeOrm/baseEntities/comment-entities/baseComment'
import { ReplyForVideo_G } from 'src/modules/replies/group-replies/entities/reply-f-video.entity'
import { Audio_F_Comment_F_Video_G } from 'src/modules/rest-files/entities/groups/entitites-for-comments/audio-f-video.entity'
import { Image_F_Comment_F_Video_G } from 'src/modules/rest-files/entities/groups/entitites-for-comments/image-f-video.entity'
import { Video_F_Comment_F_Video_G } from 'src/modules/rest-files/entities/groups/entitites-for-comments/video-f-video.entity'
import { Galery_Video_G } from 'src/modules/rest-files/entities/groups/galery-entities/video.entity'

import {
  Entity,
  Column,
  ManyToOne,
  JoinColumn,
  OneToOne,
  OneToMany,
} from 'typeorm'
import { UserEntity } from 'src/modules/users/entities/user.entity'

@ObjectType()
@Entity('comments_f_galery_video_g')
export class CommentForVideo_G extends BaseCommentEntity {
  @Column()
  ownerId: number
  @Field(() => Galery_Video_G)
  @ManyToOne(() => Galery_Video_G, (video) => video.comments, {
    onDelete: 'CASCADE',
  })
  @JoinColumn({ name: 'ownerId' })
  owner: Galery_Video_G

  @Column()
  userId: number
  @Field(() => UserEntity)
  @ManyToOne(() => UserEntity, (comment) => comment.commentForVideoEntity_G, {
    onDelete: 'CASCADE',
    eager: true,
  })
  @JoinColumn({ name: 'userId' })
  user: UserEntity

  @Field(() => Audio_F_Comment_F_Video_G)
  @OneToOne(() => Audio_F_Comment_F_Video_G, (audio) => audio.owner, {
    onDelete: 'CASCADE',
    eager: true,
  })
  audio: Audio_F_Comment_F_Video_G

  @Field(() => Image_F_Comment_F_Video_G)
  @OneToOne(() => Image_F_Comment_F_Video_G, (image) => image.owner, {
    onDelete: 'CASCADE',
    eager: true,
  })
  image: Image_F_Comment_F_Video_G

  @Field(() => Video_F_Comment_F_Video_G)
  @OneToOne(() => Video_F_Comment_F_Video_G, (video) => video.owner, {
    onDelete: 'CASCADE',
    eager: true,
  })
  video: Video_F_Comment_F_Video_G

  @Field(() => ReplyForVideo_G)
  @OneToMany(() => ReplyForVideo_G, (reply) => reply.comment, {
    onDelete: 'CASCADE',
    eager: true,
  })
  replies: ReplyForVideo_G
}
